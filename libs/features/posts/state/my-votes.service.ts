...
export class MyVotesService {
  ...
  getMyPostVotes() {
    const request = this.votesApi
      .getPostVotes()
      .pipe(tap(votes => this.myVotesStore.addPostVotes(votes)));
  ...
  getMyCommentVotes(postId: ID) {
    const request = this.votesApi
      ...
      .pipe(tap(votes => this.myVotesStore.addCommentVotes(votes, postId)));
  ...
  upvotePost(postId: ID) {
    return this.votesApi
      .upvote('posts', postId)
      .pipe(tap(vote => this.myVotesStore.addPostVote(vote)));
  }
  ...
  upvoteComment(postId: ID) {
    return this.votesApi
      ...
      .pipe(tap(vote => this.myVotesStore.addCommentVote(vote)));
  }
