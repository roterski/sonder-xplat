...
export class PostCommentsService {
  ...
  getPostComments(postId: ID) {
    const request = this.postsApi
      ...
      .pipe(
        tap((comments: PostComment[]) =>
          this.postCommentsStore.addPostComments(comments, postId)
        )
      );
    ...
    return this.postCommentsQuery.selectPostCommentsLoaded(postId).pipe(
      switchMap((loaded: boolean) => {
        return loaded
          ? this.postCommentsQuery.selectPostParentComments(postId)
          : request;
      })
    );
  }
  ...
  addPostComment(postId: ID, postComment: PostComment) {
    return this.postsApi.createComment(postId, postComment).pipe(
      tap((comment: PostComment) =>
        this.postCommentsStore.addPostComment(comment)
      ),
      catchError((err: any) => {
